...
def start_server(port):
    global server_socket, aes_key, is_running
...
def handle_client(sock, addr):
    ...
    try:
        # 发送服务器的公钥给客户端
        sock.sendall(len(public_key_pem).to_bytes(4, 'big') + public_key_pem)
        print_status(f"已发送公钥给客户端 {addr}。")

        # 接收客户端的公钥
        key_length_bytes = receive_fixed_length_data(sock, 4)
        if not key_length_bytes:
            print_status(f"无法接收客户端 {addr} 的公钥长度。")
            sock.close()
            return
        key_length = int.from_bytes(key_length_bytes, 'big')
        client_public_key_pem = receive_fixed_length_data(sock, key_length)
        ...
        sock.sendall(len(encrypted_aes_key).to_bytes(4, 'big') + encrypted_aes_key)
...
def server_receive_loop(sock, addr, client_aes_key):
    while is_running:
        try:
            ...
            # 接收加密的数据
            encrypted_data = receive_fixed_length_data(sock, data_length)
            if not encrypted_data:
                print_status(f"接收客户端 {addr} 的数据时发生错误。")
                break
            # 解密数据
            ...
            if packet_type == 'text':
                ...
                # 接收文件块
                chunk_data = bytes.fromhex(packet.get('data', ''))
                ...
                if len(file_data) >= filesize:
                    ...
                    if received_sha256 == sha256:
                        ...
                        with open(filename, 'wb') as f:
                            f.write(file_data)
                        print_status(f"文件 {filename} 接收完成并已保存。")
                    else:
                        print_status(f"文件 {filename} 校验失败，文件可能已损坏。")
            else:
...
def start_client(server_ip, server_port):
    ...
    try:
        ...
        # 接收服务器的公钥
        key_length_bytes = receive_fixed_length_data(client_socket, 4)
        if not key_length_bytes:
            print_status("无法接收服务器的公钥长度。")
            client_socket.close()
            return
        key_length = int.from_bytes(key_length_bytes, 'big')
        server_public_key_pem = receive_fixed_length_data(client_socket, key_length)
        ...
        # 发送客户端的公钥给服务器
        client_socket.sendall(len(public_key_pem).to_bytes(4, 'big') + public_key_pem)
        print_status("已发送公钥给服务器。")

        # 接收加密的 AES 密钥并解密
        encrypted_key_length_bytes = receive_fixed_length_data(client_socket, 4)
        if not encrypted_key_length_bytes:
            print_status("无法接收加密的 AES 密钥长度。")
            client_socket.close()
            return
        encrypted_key_length = int.from_bytes(encrypted_key_length_bytes, 'big')
        encrypted_aes_key = receive_fixed_length_data(client_socket, encrypted_key_length)
    ...
    while is_running:
        try:
            if send_queue:
                ...
                client_socket.sendall(len(encrypted_data).to_bytes(4, 'big') + encrypted_data)
            else:
                time.sleep(0.1)
        except Exception as e:
...
def client_receive_loop():
    ...
    while is_running:
        try:
            # 接收数据包长度
            length_bytes = receive_fixed_length_data(client_socket, 4)
            if not length_bytes:
                print_status("服务器已断开连接。")
                break
            data_length = int.from_bytes(length_bytes, 'big')
            # 接收加密的数据
            encrypted_data = receive_fixed_length_data(client_socket, data_length)
            if not encrypted_data:
                print_status("接收服务器的数据时发生错误。")
                break
            # 解密数据
                        ...
                        with open(filepath, 'rb') as f:
                            while True:
                                ...
                                if not data:
                                    break
                                data_hex = data.hex()
                                file_chunk_packet = create_packet('file_chunk', {'data': data_hex})
