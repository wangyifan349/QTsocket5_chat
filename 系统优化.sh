...
# 12. 禁用不需要的用户账户（如Ubuntu默认账户）
echo "禁用不必要的用户账户..."
sudo usermod -L ubuntu

# 13. 配置永久禁用虚拟内存（通过 /etc/fstab 和 /etc/rc.local）
echo "配置永久禁用虚拟内存..."
...
# 15. 设置加密DNS（使用 systemd-resolved 配置 DoH 或 DoT）
echo "配置加密DNS..."
...
# 16. 配置代理（HTTP、HTTPS 和 SOCKS5 代理）
echo "配置代理设置..."
...
# 18. 安装基础开发环境：git, curl, gcc, make, build-essential
echo "安装基础开发环境..."
sudo apt-get install -y git curl gcc make build-essential

# 19. 安装 C 语言编译环境：gcc 和相关工具
...
# 23. 安装其他基础工具：wget, unzip, zip, tar
echo "安装其他常用工具..."
